		...
		for (Edge edge : graph.outgoingEdgesOf(node)) {
			...
			if(!triple.predicateMatches(RDF.type.asNode())){
				...
				if(varTypes.get(triple.getSubject()).iterator().next() == LITERAL_NODE_TYPE){
					...
					NPPhraseSpec phrase = triplePatternConverter.convertTriplePattern(edge.asTriple(), subjectElement, objectElement, plural, false, edge.isReverted(), NounPredicateConversion.RELATIVE_CLAUSE_PRONOUN);
					phrases.add(phrase);
